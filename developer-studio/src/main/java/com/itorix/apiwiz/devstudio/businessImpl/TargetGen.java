...
public class TargetGen {
	...
	public void generateTargetCode(Folder targetFolder, CodeGenHistory cg, String dir)
		...
		for (Target target : targets) {
			...
			dstRootFolder = dir + "API" + File.separatorChar + "Targets" + File.separatorChar + targetName
					+ File.separatorChar + cg.getProxy().getVersion();
	...
	private void processTargetEndpointTemplate(Flows flows, Folder target, CodeGenHistory cg)
		...
		for (Folder tmplFile : files) {
			if (!tmplFile.isFolder()) {
				...
				String dstFileName = dstRootFolder + File.separatorChar + "target" + File.separatorChar + tgtfilePrefix
	...
	private void processFlowFragTemplates(Flows flows, Folder fragments) throws IOException, TemplateException {
		...
		for (Folder fragFile : fileList) {
			...
			if (fragFile.getName().contains(STR_TARGET)) {
				...
				dstFile = dstFile.replace(STR_TARGET, ProxyConfig.EMPTY_STRING).replace(ProxyConfig.FTL_FILE_EXT,
						ProxyConfig.EMPTY_STRING);
				targetDetails.put("targetName", targetName);
				...
				for (int i = 0; i < flows.getFlow().length; i++) {
					final Map<String, Object> operationDetails = new HashMap<String, Object>();
					// File output
					dstFile = dstFragflows + File.separatorChar + targetName + "_" + flows.getFlow()[i].getName() + "_"
							+ fragFile.getName();
