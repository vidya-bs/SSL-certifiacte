...
public class MongoConnection {
	...
	public ProxyHistoryResponse getProxyHistory(int offset, int pageSize, String proxy){
		...
		try {
			...
			if(proxy != null)
				...
				query = new Query().with(Sort.by(Direction.DESC, "dateModified")).skip(offset > 0 ? ((offset - 1) * pageSize) : 0).limit(pageSize);
	...
	public ProxyData saveProxyDetailsByOrgEnv(String proxy,String org, String env, String type) throws JsonParseException, JsonMappingException, IOException{
		ProxyData proxyData = getProxyHistory(proxy);
	...
	public ProxyData saveProxyDetailsByOrgEnv(String proxy,OrgEnv orgEnv) throws JsonParseException, JsonMappingException, IOException{
		String deployedStatus = "";
		ProxyData proxyData = getProxyHistory(proxy);
		for(Env env : orgEnv.getEnvs()){
			ProxyApigeeDetails proxyApigeeDetails = apigeeDetails.getDetailsByProxy(proxyData, orgEnv.getName(), env.getName(), orgEnv.getType());
			proxyData.setProxyApigeeDetails(proxyApigeeDetails);
			...
			try{
				for(Deployments deployment: deployments){
					if(deployment.getOrg().equals(orgEnv.getName())&&deployment.getEnv().equals(env.getName())){
						List<com.itorix.apiwiz.common.model.proxystudio.apigeeassociations.Proxy> proxiesList = deployment.getProxies();
						com.itorix.apiwiz.common.model.proxystudio.apigeeassociations.Proxy deployedProxy = proxiesList.get(0);
	...
	public String getResourceFile(String projectName, String proxyName, String type, String fileName, String destinationLocation){
		...
		try {
			reader= reader.replaceAll("schemaLocation=\"", "schemaLocation=\"xsd://").replaceAll("\" location=\"", "\" location=\"wsdl://");
	...
	public String findProjectFile(
			String projectName, String proxyName, String type, String fileName) {
