...
public class ProjectManagementDao {
	...
	public Object createNewProject(Project project, String jsessionid) throws ItorixException {
		...
		if (projectexists == null) {
			 ...
			 * =userSessionRepository.findOne(jsessionid); User dbUser =
			 * mongoOperations.findById(userSessionToken.getUserId(),User.class)
			 * ;
			 */
			...
			if (project.getProxies() != null && project.getProxies().size() > 0) {
				for (Proxies proxy : project.getProxies()) {
					...
					if (proxy.getXsdFiles() != null) {
						proxy.setXsdFiles(
								updateProxyFiles(project.getName(), proxy.getName(), proxy.getXsdFiles(), "XSD"));
					}
					if (proxy.getWsdlFiles() != null) {
						proxy.setWsdlFiles(
								updateProxyFiles(project.getName(), proxy.getName(), proxy.getWsdlFiles(), "WSDL"));
					}
					if (proxy.getAttachments() != null) {
						proxy.setAttachments(updateProxyFiles(project.getName(), proxy.getName(),
								proxy.getAttachments(), "ATTACHMENT"));
					}
				}
			}
	...
	public Object findProjectByName(String projectName) throws ItorixException {
		...
		if (particularProject != null) {
			...
			throw new ItorixException(String.format(ErrorCodes.errorMessage.get("ProjectPlan-1001"), projectName),
					"ProjectPlan-1001");
		}
	}
	...
	public Object getOverview(int offset) throws ItorixException {
		Query query = new Query().with(Sort.by(Direction.DESC, "mts")).skip(offset > 0 ? ((offset - 1) * 10) : 0)
				.limit(10);
	...
	public Object updateProject(Project project, String jsessionId) throws ItorixException {
		...
		if (projectexists != null) {
			...
			throw new ItorixException(String.format(ErrorCodes.errorMessage.get("ProjectPlan-1001"), project.getName()),
					"ProjectPlan-1001");
		}
		return "";
	}
	...
	public ProjectFile findProjectFile(ProjectFile projectFile) {
		Query query = new Query(Criteria.where("projectName").is(projectFile.getProjectName()).and("proxyName")
				.is(projectFile.getProxyName()).and("type").is(projectFile.getType()).and("name")
				.is(projectFile.getFileName()));
	...
	public void updateProjectFileInUse(ProjectFile projectFile) {
		Query query = new Query(Criteria.where("projectName").is(projectFile.getProjectName()).and("proxyName")
				.is(projectFile.getProxyName()).and("type").is(projectFile.getType()).and("fileName")
				.is(projectFile.getFileName()));
	...
	public List<ProjectFile> findProjectFiles(String projectName, String proxyName, String type) {
		Query query = new Query(
				Criteria.where("projectName").is(projectName).and("proxyName").is(proxyName).and("type").is(type));
		List<ProjectFile> files = mongoTemplate.find(query, ProjectFile.class);
		return files;
	}

	public List<ProjectFile> updateProxyFiles(String projectName, String proxyName, List<ProjectFile> files,
			String type) {
		List<ProjectFile> dbFiles = this.findProjectFiles(projectName, proxyName, type);
		for (ProjectFile dbFile : dbFiles) {
			boolean found = false;
			for (ProjectFile file : files) {
				String fileLocation = applicationProperties.getAppUrl() + "/" + applicationProperties.getAppDomain()
						+ "/" + "v1/projects/" + projectName + "/proxy/" + proxyName + "/" + type.toLowerCase() + "/"
						+ file.getFileName();
	...
	public void createUpdateServiceRegistry(String projectName, String proxyName, ServiceRegistry serviceRegistry) {
		if (getServiceRegistry(projectName, proxyName, serviceRegistry.getName()) != null) {
			...
			if (project.getProxies() != null)
				for (Proxies proxy : project.getProxies())
					if (proxy.getName().equals(proxyName)) {
						List<ServiceRegistry> serviceRegistries = proxy.getServiceRegistries() == null
								? new ArrayList<ServiceRegistry>()
								: proxy.getServiceRegistries();
	...
	public List<Category> getCategories() throws ItorixException {
		try {
			...
			List<com.itorix.apiwiz.common.model.proxystudio.Category> categories = mongoTemplate
					.findAll(com.itorix.apiwiz.common.model.proxystudio.Category.class);
			ObjectMapper mapper = new ObjectMapper();
	...
	public Map<String, String> loadExcelData(MultipartFile file, String jsessionId)
			throws IOException, ItorixException, InvalidFormatException {
		...
		for (Map<String, String> map : excelReader.readExcelData(excelFile)) {
			try {
				...
				if (project != null) {
					project.getProxies().get(0)
							.setPolicyTemplates(excelReader.populatePolicyTemplates(map, getCategories()));
	...
	public void loadProxyData(MultipartFile file, String projectName, String jsessionId) {
		...
		String zipLocation = applicationProperties.getTempDir() + UUID.randomUUID().toString() + File.separatorChar
				+ "unzip" + File.separatorChar;
		...
		try {
			...
			if (project != null) {
				...
				File[] attachments = new File(
						zipLocation + zipFileName + File.separatorChar + "attachments" + File.separatorChar)
								.listFiles();
	...
	public ProjectProxyResponse generateProxy(String projectName, String proxyName, String jsessionId) {
		...
		try {
			...
			if (project.getProxyByName(proxyName).getDefaultVirtualHosts()) {
				...
				if (mapping != null)
					if (project.getProxyByName(proxyName).getApigeeVirtualHosts() != null)
						project.getProxyByName(proxyName).getApigeeVirtualHosts()
								.addAll(Arrays.asList(mapping.getValue().split(",")));
					else {
			...
			projectBusinessImpl.createServiceConfigs(projectName, proxyName,
	...
	private List<com.itorix.apiwiz.projectmanagement.model.cicd.Stage> populateStages(Project project,
			String branchtype) {
			...
			if (pipeline.getBranchType().equals(branchtype))
				for (Stage stage : pipeline.getStages()) {
					...
					if (stage.getCodeCoverage().getTestsuites() != null) {
						...
						testSuiteAndConfig
								.setEnvironmentId(stage.getCodeCoverage().getTestsuites().get(0).getEnvironmentId());
	...
	private CodeGenHistory populateProxyGenerationObj(Project project) throws ItorixException {
		try {
			...
			for (Category devCategory : categories) {
				...
				com.itorix.apiwiz.common.model.proxystudio.Category category = mapper.readValue(stringCategory,
						com.itorix.apiwiz.common.model.proxystudio.Category.class);
				listCategory.add(category);
			}
			CodeGenHistory codeGenHistory = new CodeGenHistory();
			// codeGenHistory.setPolicyTemplates(getTemplates());//remove this
			// line
	...
	private List<com.itorix.apiwiz.common.model.proxystudio.Category> getTemplates(List<Category> policyTemplates) {
		ObjectMapper mapper = new ObjectMapper();
		// String templateStr =
		// "{\"policyTemplates\":[{\"type\":\"trafficmanagement\",\"name\":\"TrafficManagement\",\"description\":\"Traffic
		// management policies let you configure cache, control traffic quotas
		// and spikes, set concurrent rate limits,
		// and so
		// on.\",\"policies\":[{\"name\":\"spikearrest\",\"displayName\":\"SpikeArrest\",\"description\":\"Spike
		// Arrest
		// policy protects against traffic
		// spikes.\",\"enabled\":true},{\"name\":\"quotaatproductlevel\",\"displayName\":\"Quota
		// at
		// Productlevel\",\"description\":\"Quota policy to configure the
		// number of request messages that an API proxy allows over a period of
		// time.\",\"enabled\":false},{\"name\":\"responsecache\",\"displayName\":\"Response
		// Cache\",\"description\":\"Caches data from a backend resource,
		// reducing the number of requests to the
		// resource.\",\"enabled\":false}]},{\"type\":\"security\",\"name\":\"Security\",\"description\":\"Security
		// policies let you control access to your APIs with OAuth, API key
		// validation, and other threat protection
		// features.\",\"policies\":[{\"name\":\"oauth\",\"displayName\":\"OAuth\",\"description\":\"Secure
		// APIs with
		// OAuth\",\"enabled\":false},{\"name\":\"cors\",\"displayName\":\"CORS\",\"description\":\"Enable
		// API's to
		// support CORS
		// requests\",\"enabled\":false},{\"name\":\"accesscontrol\",\"displayName\":\"Access
		// Control\",\"description\":\"Lets you allow or deny access to your
		// APIs by specific IP addresses.
		// \",\"enabled\":false},{\"name\":\"verifyapikey\",\"displayName\":\"Verify
		// API Key\",\"description\":\"Lets
		// you
		// enforce verification of API keys at
		// runtime.\",\"enabled\":false},{\"name\":\"jwt\",\"displayName\":\"JWT\",\"description\":\"Lets
		// you enforce
		// JWT verification for your
		// proxy.\",\"enabled\":false}]},{\"type\":\"logging\",\"name\":\"Logging\",\"description\":\"Logging
		// policies
		// allow you to log
		// the request data and response
		// data.\",\"policies\":[{\"name\":\"log\",\"displayName\":\"Logger\",\"description\":\"Lets
		// you to log the
		// proxy data during
		// runtime.\",\"enabled\":false}]},{\"type\":\"mediation\",\"name\":\"Mediation\",\"description\":\"Mediation
		// policies let you perform message
		// transformation, parsing, and validation, as well as raise faults and
		// alerts.\",\"policies\":[{\"name\":\"jsontoxml\",\"displayName\":\"JSON
		// To XML\",\"description\":\"Converts
		// messages from the JavaScript Object Notation (JSON) format to
		// extensible markup language (XML)
		// \",\"enabled\":false},{\"name\":\"xmltojson\",\"displayName\":\"XML
		// To JSON\",\"description\":\"Converts
		// messages from the extensible markup language (XML) fromat to
		// JavaScript Object Notation
		// (JSON).\",\"enabled\":false}]},{\"type\":\"threatprotection\",\"name\":\"ThreatManagement\",\"description\":\"ThreatManagement
		// allow you to scan the incoming payloads for any miscellaneous content
		// in the
		// request.\",\"policies\":[{\"name\":\"jsoninjection\",\"displayName\":\"JSON
		// Injection\",\"description\":\"Enable payload scan for JSON
		// content\",\"enabled\":false},{\"name\":\"xssinjection\",\"displayName\":\"XSS
		// Injection\",\"description\":\"Scans XSS threats in
		// payloads\",\"enabled\":false},{\"name\":\"xmlinjection\",\"displayName\":\"XML
		// Injection\",\"description\":\"Enable payload scan for XML
	...
	public void createPromotePipeline(ScmPromote scmPromote, String projectName, String proxyName, String jsessionid)
		...
		pipelineGroups.setPipelines(pipelines);
		String branch = pipeline.getMaterials().get(0).getScmBranch();
		// if(!isPipelineExists(project.getName().replaceAll(" " ,
		// "-").replaceAll("\\." , ""), branch,
		// jsessionid))
		createPipeline(pipelineGroups, jsessionid);
		projectBusinessImpl.createServiceConfigs(projectName, proxyName, getBranchType(scmPromote.getTargetBranch()),
				jsessionid);
	}

	public void promoteToMaster(ScmPromote scmPromote, String projectName, String proxyName, String jsessionid)
