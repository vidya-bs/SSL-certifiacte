package com.itorix.apiwiz.test.executor.exception;
 ...
 * 
 * 
 * The {@link GenericControllerExceptionHandler} handles exceptions thrown in the controller layer. It catches the
...
public class GenericControllerExceptionHandler {
    ...
    public ResponseEntity<ErrorObj> handleControllerException(final ItorixException ex,
        ...
        ResponseEntity<ErrorObj> responseEntity = new ResponseEntity<ErrorObj>(error,
                HttpStatus.valueOf(ErrorCodes.responseCode.get(ex.errorCode)));
        return responseEntity;
    }
    ...
    /*
     * @ExceptionHandler(Exception.class) public final ResponseEntity<ErrorObj> handleAllExceptions(Exception ex,
     * WebRequest request) { ErrorObj errorDetails = new ErrorObj("", ex.getMessage()); return new
     * ResponseEntity<>(errorDetails, HttpStatus.INTERNAL_SERVER_ERROR); }
     * 
     * @ExceptionHandler(ItorixException.class) public final ResponseEntity<ErrorObj>
     * handleUserNotFoundException(ItorixException ex, WebRequest request) { ErrorObj errorDetails = new ErrorObj("",
     * ex.getMessage()); return new ResponseEntity<>(errorDetails, HttpStatus.NOT_FOUND); }
     */
