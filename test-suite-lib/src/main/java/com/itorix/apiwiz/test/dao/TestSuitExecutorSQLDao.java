...
public class TestSuitExecutorSQLDao {
    ...
    public int updateStatusForTestExecutionId(String id, String value) {
        return jdbcTemplate.update("update " + TestExecutorEntity.TABLE_NAME
                + " set status = ? where testSuiteExecutionId = ? and status <> 'Cancelled'", value, id);
    }

    public String getValueByColumnName(String id, String columnName) {
        String query = "select " + columnName + " from " + TestExecutorEntity.TABLE_NAME
