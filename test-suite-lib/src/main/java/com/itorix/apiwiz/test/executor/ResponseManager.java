...
public class ResponseManager {
    ...
    public ResponseValidator gatherResponseData(HttpResponse actualResponse, Response response,
            Map<String, String> vars, Map<String, String> encryptedVariables, MaskFields maskFields)
        ...
        if (response.getVariables() != null) {
            ...
            for (Variable variable : response.getVariables()) {
                if (variable.getReference() != null) {
                    ...
                    if ("Header".equalsIgnoreCase(reference) || "headers".equalsIgnoreCase(reference)) {
                        ...
                        if (variable.isEncryption()) {
                            try {
                                headerMap.put(variable.getName(),
                                        (new RSAEncryption()).encryptText(headerMap.get(variable.getName())));
                            } catch (Exception e) {
                        ...
                        try {
                            ...
                            if (variable.isEncryption()) {
                                validator.setAttributeValue(variable.getValue(),
                                        (new RSAEncryption()).encryptText(vars.get(variable.getName())));
                            }
                        } catch (Exception e) {
                            logger.error("error while encrypting text", e);
                        }
                ...
                if (variable.isEncryption()) {
                    try {
                        encryptedVariables.put(variable.getName(),
                                (new RSAEncryption()).encryptText(vars.get(variable.getName())));
                    } catch (Exception e) {
                        logger.error("error while encrypting text", e);
                    }
