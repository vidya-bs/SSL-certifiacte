...
public class TestRunner {
	...
	public void run(ExecutionContext context) {
		...
		try {
			...
			finalResponse = executeTestSuite(response.getId() , testSuite, variables, false, false,context.getGlobalTimeout());
			...
			sqlDao.updateStatusForTestExecutionId(testExecutorEntity.getTestSuiteExecutionId(), TestExecutorEntity.STATUSES.COMPLETED.getValue());
			}
		}catch(Exception ex) {
	...
	public TestSuiteResponse executeTestSuite(String testSuiteResponseID,TestSuite testSuite, Variables vars, Boolean skipAssertions, Boolean isMonitoring,int globalTimeout) {
		...
		if (testSuite.getScenarios() != null) {
			for (Scenario scenario : testSuite.getScenarios()) {
				...
				if(cancellationExecutor.getAndRemoveTestSuiteCancellationId(testSuiteResponseID)){
					return new TestSuiteResponse(testSuite.getId(), null, testSuite, TestExecutorEntity.STATUSES.CANCELLED.getValue());
				}
					...
					for (; succededTests.size() + failedTests.size() < numberOfTestCases; counter++) {
						...
						try {
							if (canExecuteTestCase(testCase, succededTests, failedTests)) {
								...
								//creating maskingFields object in case it is null, just to avoid null checks at many places
								if(maskingFields == null){
									maskingFields = new MaskFields();
								}
								invokeTestCase(testCase, globalVars, encryptedVariables, maskingFields ,testStatus , skipAssertions, isMonitoring, sslConnectionFactory , globalTimeout);
							} else {
								counter++;
			...
			if (failedScenarios.isEmpty()) {
				...
				logger.debug(" number of failedscenarios in testSuiteResponseID {} , {} " , testSuiteResponseID , failedScenarios.size());
...
//			}
					...
					if (scenario != null && scenario.getTestCases() != null) {
						...
						for (TestCase testCase : testCases) {
							...
							for (Variable variable : variables) {
								...
								if (!CollectionUtils.isEmpty(maskingFieldsValue) && maskingFieldsValue.contains(variable.getName()) && StringUtils.hasText(value)) {
			...
			testSuite.setSuccessRate((int)Math.round(Double.valueOf(scenarioSuccessSum / testSuite.getScenarios().size())));
			testSuite.setVars(encryptedVariableHeaders);
			testSuite.setDate(java.time.LocalDateTime.now().toString());
		}
		return new TestSuiteResponse(testSuite.getId(), null, testSuite, TestExecutorEntity.STATUSES.COMPLETED.getValue());
	}
...
//	}
	...
	/**
	 *
	 * Usecases to be covered
	 * 1. No dependencies - First Time Execution - Loop Execution
	 * 2. Having Dependencies - First Time Execution - Loop Execution
	 * 3. Dependent testcase passed 4. Dependent testcase failed 5. Dependent testcase didn't execute
	 *
	 * @param testCase
	 * @param succededTests
	 * @param failedTests
	...
	public void invokeTestCase(TestCase testCase, Map<String, String> globalVars,  Map<String, String> encryptedVariables,MaskFields  maskingFields ,
			Map<String, Integer> testStatus, Boolean skipAssertion, Boolean isMonitoring, SSLConnectionSocketFactory sslConnectionFactory , int globalTimeout) throws Exception {
		...
		if (testCase.getVerb() != null) {
			...
			if (!CollectionUtils.isEmpty(testCase.getRequest().getQueryParams())) {
				clonedQueryParams = mapper.readValue(
						mapper.writeValueAsString(testCase.getRequest().getQueryParams()),
			...
			int timeout = testCase.getTimeout()!= null && testCase.getTimeout() > 0 ? testCase.getTimeout().intValue() : globalTimeout;
			if (path != null) {
			...
			List<Header> clonedHeaders = mapper.readValue(mapper.writeValueAsString(header), new TypeReference<List<Header>>() {
			});
			...
			if (testCase.getRequest() != null && testCase.getRequest().getBody() != null
				...
				if(maskingFields!=null && !maskingFields.getMaskingFields().isEmpty()){
					reqBodyToSet = MaskFieldUtil.getMaskedResponseForJson(maskingFields.getMaskingFields(), reqBodyToSet);
				}
			...
			if (testCase.getVerb().equalsIgnoreCase(API.GET.toString())) {
				...
				if (testCase.getRequest() != null
						&& (!CollectionUtils.isEmpty(testCase.getRequest().getFormParams())
					...
					if (!CollectionUtils.isEmpty(testCase.getRequest().getFormParams())) {
						...
						List<FormParam> formParamClone = mapper.readValue(mapper.writeValueAsString(testCase.getRequest().getFormURLEncoded()), new TypeReference<List<FormParam>>() {
						});


						generateNameValuePairs = generateNameValuePairs(formParamClone,
								globalVars);
				...
				response = APIFactory.invokeDelete(path, headers, reqBody, testCase.getName(),sslConnectionFactory,timeout);
			} else if (testCase.getVerb().equalsIgnoreCase(API.OPTIONS.toString())) {
				response = APIFactory.invokeOptions(path, headers, reqBody, testCase.getName(),sslConnectionFactory,timeout);
			} else if (testCase.getVerb().equalsIgnoreCase(API.PATCH.toString())) {
				response = APIFactory.invokePatch(path, headers, reqBody, testCase.getName(),sslConnectionFactory,timeout);
			}
			...
			validateResponse(response, testCase.getResponse(), testCase.getName(), globalVars, encryptedVariables ,testStatus,
					skipAssertion,maskingFields);
...
//					});
	...
	private Result validateResponse(HttpResponse actualResponse, Response expectedResponse, String testCaseId,
			Map<String, String> globalVars, Map<String, String> encryptedVariables, Map<String, Integer> testStatus, Boolean skipAssertions,MaskFields maskingFields) throws Exception {
		...
		ResponseValidator validator = responseManager.gatherResponseData(actualResponse, expectedResponse, globalVars,encryptedVariables,maskingFields);
	...
	private String computeQueryParams(String path, List<QueryParam> queryParams,
	...
	private void replaceResponseVariables( Response response,
			HttpResponse httpResponse, MaskFields maskingFields) {
