...
public class TestsuiteServiceImpl implements TestSuiteService {
	...
	public ResponseEntity<?> triggerTestSuite(String testSuiteId, String variableId,
		...
		try {
			...
			if (isSchedulerCall) {
				...
				User user = commonServices.getUserDetailsFromSessionID(jsessionId);
				testSuitRespId = dao.executeTestSuite(testSuiteId, variableId, user, false);
	...
	public ResponseEntity<?> cancelTestSuite(@PathVariable("testsuiteId") String testSuiteId,
		...
		for (TestSuiteResponse testSuite : cancelTestSuite) {
			...
			try {
				log.debug("Making a call to {}", testSuitAgentPath + testSuitContextPath);
				result = restTemplate.postForEntity(testSuitAgentPath + testSuitContextPath + TEST_SUITE_CANCEL,
