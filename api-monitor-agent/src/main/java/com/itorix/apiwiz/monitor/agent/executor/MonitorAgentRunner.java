    ...
    public void executeMonitorRequests(ExecutionContext context, int globalTimeout) {
        ...
        for (String sequence : requestSequence) {
            ...
            try {
                ...
                response = monitorAgentHelper.invokeMonitorApi(monitorRequest, globalVars, encryptedVariables,
                        testStatus, sslConnectionFactory, globalTimeout);
        ...
        for (Notifications notification : notificationDetail.getNotifications()) {
            ...
            try {
                if (!CollectionUtils.isEmpty(notification.getEmails())) {
                    ...
                    String mailSubject = emailContentParser.getEmailSubject(notificationType,
                            notificationDetail.getWorkspaceName(), notificationDetail.getCollectionname(),
                            notificationDetail.getEnvironmentName());
