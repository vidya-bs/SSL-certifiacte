...
public class MonitorAgentExecutorSQLDao {
	...
	public List<MonitorAgentExecutorEntity> getExecutorEntityByColumn(String columnName, Object columnValue, int count) {
	...
	public int updateField(Long id, String field, String value) {
		return jdbcTemplate.update("update " + MonitorAgentExecutorEntity.TABLE_NAME + " set " + field + " = ? where id = ?",
				value, id);
	}
	...
	public void updateStatus(List<String> statuses, String newStatus) {
		...
		namedParameterJdbcTemplate.update(
				"update " + MonitorAgentExecutorEntity.TABLE_NAME + " set status = :newStatus WHERE status IN (:status)",
				parameters);

	}

	public void insertIntoTestExecutorEntity(String tenant, String collectionId , String schedulerId, String status) throws ItorixException {
		if (!StringUtils.hasText(tenant) || !StringUtils.hasText(collectionId) || !StringUtils.hasText(status) || !StringUtils.hasText(schedulerId)) {
		...
		jdbcTemplate.update("insert into " + MonitorAgentExecutorEntity.TABLE_NAME
				+ " (tenant, collection_id , scheduler_id , status) values(?,?,?,?)", tenant, collectionId, schedulerId ,status);
	}
	...
	public String getValueByColumnName(String id, String columnName) {
		String query = "select " + columnName +  " from " + MonitorAgentExecutorEntity.TABLE_NAME + " where testSuiteExecutionId = ? ";
