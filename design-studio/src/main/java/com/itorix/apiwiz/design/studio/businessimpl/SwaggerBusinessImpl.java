...
public class SwaggerBusinessImpl implements SwaggerBusiness {
  ...
  public boolean saveSwagger(SwaggerVO swaggerVO) throws ItorixException {
    ...
    if (vo != null) {
      throw new ItorixException(
          String.format(ErrorCodes.errorMessage.get("Swagger-1002"), swaggerVO.getName()),
          "Swagger-1002");
    }
    swaggerVO.setRevision(1);
    swaggerVO.setStatus("Draft");
    ...
    private List<SwaggerImport> importSwaggersFromFiles(List<File> files,String oas) throws Exception {
        ...
        for (File file : files) {
            try {
				...
				if (isVersion2) {
					logger.debug("Adding swaggers to swaggers list");
   ...
   * @return
   */
  public SwaggerVO createSwaggerWithNewRevision(SwaggerVO swaggerVO, String jsessionid)
    ...
    List<Revision> revisions = getListOfRevisions(swaggerVO.getName(),
        swaggerVO.getInteractionid());
   ...
   * @return
   */
  public Swagger3VO createSwaggerWithNewRevision(Swagger3VO swaggerVO, String jsessionid)
      throws ItorixException {
    log("createSwaggerWithNewRevision", swaggerVO.getInteractionid(), swaggerVO);
    ...
    List<Revision> revisions = getListOf3Revisions(swaggerVO.getName(),
        swaggerVO.getInteractionid());
    if (revisions != null && revisions.size() > 0) {
      Revision revision = Collections.max(revisions);
      Integer newRevision = revision.getRevision() + 1;
  ...
  public SwaggerVO findSwagger(SwaggerVO swaggerVO, Integer revision) {
    ...
    return baseRepository.findOne("name", swaggerVO.getName(), "revision", revision,
        SwaggerVO.class);
  }
  ...
  public List<Revision> getListOf3Revisions(String name, String interactionid) {
	...
	public List<Revision> getListOfRevisions(String name, String status, String interactionid) {
		...
		List<Revision> versions = new ArrayList<Revision>();
		if (swaggers != null && swaggers.size() > 0) {
			logger.debug("Adding swagger deatils to version");
			for (SwaggerVO swagger : swaggers) {
				Revision version = new Revision();
				version.setRevision(swagger.getRevision());
				version.setStatus(swagger.getStatus());
  ...
  private void getSwaggers() {
    ...
    AggregationResults<Object> result = mongoTemplate.aggregate(aggregation, SwaggerVO.class,
        Object.class);
  ...
  public SwaggerHistoryResponse getListOfSwaggerDetails(String status, String modifiedDate,
    ...
    if (isAdmin) {
      ...
      List<String> trimList = trimList(
          baseRepository.filterAndGroupBySwaggerName(filterFieldsAndValues,
  ...
  public SwaggerHistoryResponse getSwaggerDetailsByproduct(List<String> products,
      String interactionid,
      String jsessionid, int offset, String oas, String swagger, int pageSize)
      throws ItorixException, JsonProcessingException, IOException {
    SwaggerHistoryResponse response = new SwaggerHistoryResponse();
  ...
  private SwaggerVO getSwaggerDetails(String swagger)
      throws JsonMappingException, JsonProcessingException {
    List<Revision> versions = getListOfRevisions(swagger, null);
    SwaggerVO vo = null;
    if (versions != null && versions.size() > 0) {
      Revision revision = Collections.max(versions);
      vo = baseRepository.findOne("name", swagger, "revision", revision.getRevision(),
          SwaggerVO.class);
    }
    if (vo != null) {
      ObjectMapper mapper = new ObjectMapper();
  ...
  private Swagger3VO getSwagger3Details(String swagger)
      throws JsonMappingException, JsonProcessingException {
    List<Revision> versions = getListOfSwagger3Revisions(swagger, null);
    Revision revision = Collections.max(versions);
    Swagger3VO vo = baseRepository.findOne("name", swagger, "revision", revision.getRevision(),
        Swagger3VO.class);
    if (vo != null) {
      ObjectMapper mapper = new ObjectMapper();
      JsonNode swaggerJson = mapper.readTree(vo.getSwagger());
  ...
  public int getSwagger3Count(String status) {
    ...
    if (status != null) {
      names = getList(mongoTemplate.getCollection(mongoTemplate.getCollectionName(Swagger3VO.class))
          .distinct("name", new Query(new Criteria(STATUS_VALUE).is(status)).getQueryObject(),
              String.class));
    } else {
   ...
   */
  @SuppressWarnings("unchecked")
  public SwaggerHistoryResponse getListOfSwagger3Details(String status, String modifiedDate,
      String interactionid,
      String jsessionid, int offset, String oas, String swagger, int pageSize,
      String sortByModifiedDate)
      throws ItorixException, JsonProcessingException, IOException {
  ...
  private void getPublishedSwaggerDetails(SwaggerVO vo, ArrayNode arrayNode)
    ...
    if (null == jsonNode) {
      ...
      String url =
          applicationProperties.getAppUrl() + applicationProperties.getAppDomain() + "/v1/swaggers/"
   ...
   * @throws JsonProcessingException
   */
  public ArrayNode getListOfPublishedSwagger3Details(String interactionid, String jsessionid,
      String status,
      String partnerId) throws ItorixException, JsonProcessingException, IOException {
    log("getListOfPublishedSwaggerDetails", interactionid, jsessionid);
   ...
   * @return
   */
  public Swagger3VO getSwagger3WithVersionNumber(String name, Integer revision,
      String interactionid)
      throws ItorixException {
    log("getSwaggerWithVersionNumber", interactionid, name, revision);
  ...
  public Boolean getSwagger3LockStatus(String name, Integer revision, String interactionid) {
    log("getLockStatus", interactionid, name, revision);
    Swagger3VO swaggerVO = baseRepository.findOne("name", name, "revision", revision,
        Swagger3VO.class);
    log("getLockStatus", interactionid, swaggerVO);
    return swaggerVO.getLock();
  }
   ...
   */
  public SwaggerVO updateStatus(String name, Integer revision, String json, String interactionid,
      String jsessionid)
  ...
  public List<String> getSwaggerRoles(String name, String oas, String interactionid,
      String jsessionid)
      throws ItorixException {
    UserSession userSessionToken = ServiceRequestContextHolder.getContext().getUserSessionToken();
    User user = getUserDetailsFromSessionID(jsessionid);
   ...
   * @throws JSONException
   */
  public Swagger3VO updateSwagger3Status(String name, Integer revision, String json,
      String interactionid,
   ...
   * @throws MessagingException
   */
  private String getStatus(String status, String previousStatus, Swagger3VO vo)
      throws MessagingException {
   ...
   * @throws MessagingException
   */
  private void sendEmails(SwaggerVO vo, String status, String previousStatus)
      throws MessagingException {
    String subject = MessageFormat.format(applicationProperties.getSwaggerChangeStatusSubject(),
        vo.getName());
    User user = getUserDetailsFromSessionID(
        ServiceRequestContextHolder.getContext().getUserSessionToken().getId());
    String userName = "";
    if (user != null) {
    ...
    String body = MessageFormat.format(applicationProperties.getSwaggerChangeStatusBody(),
        vo.getName(), status,
        getSwaggerCountbyStatus(status), previousStatus, getSwaggerCountbyStatus(previousStatus),
        userName,
  ...
  private void sendEmails(Swagger3VO vo, String status, String previousStatus)
      throws MessagingException {
    String subject = MessageFormat.format(applicationProperties.getSwaggerChangeStatusSubject(),
        vo.getName());
    User user = getUserDetailsFromSessionID(
    ...
    String body = MessageFormat.format(applicationProperties.getSwaggerChangeStatusBody(),
        vo.getName(), status,
        getSwagger3CountbyStatus(status), previousStatus, getSwagger3CountbyStatus(previousStatus),
        userName,
  ...
  public void updateLockStatus(SwaggerVO swaggerVO, String jsessionid) {
    ...
    SwaggerVO vo = baseRepository.findOne("name", swaggerVO.getName(), "revision",
  ...
  public void updateSwagger3LockStatus(Swagger3VO swaggerVO, String jsessionid) {
    log("updateLockStatus", swaggerVO.getInteractionid(), swaggerVO);
    Swagger3VO vo = baseRepository.findOne("name", swaggerVO.getName(), "revision",
        swaggerVO.getRevision(),
        Swagger3VO.class);
    if (swaggerVO.getLock()) {
      User user = getUserDetailsFromSessionID(jsessionid);
   ...
   * @throws Exception
   */
  public String genarateXpath(MultipartFile xsdFile, String elementName, String interactionid)
      throws Exception {
   ...
   * @throws Exception
   */
  public SwaggerVO genarateSwaggerDefinations(SwaggerVO swaggerVO, MultipartFile xpathFile,
      String sheetName,
    ...
    if (revision == null) {
      ...
      swaggerVO = baseRepository.findOne("name", swaggerVO.getName(), "revision", revision,
          SwaggerVO.class);
      if (swaggerVO != null) {
   ...
   * @throws Exception
   */
  public SwaggerVO genarateSwaggerJsonDefinations(SwaggerVO swaggerVO, List<RowData> rowDataList,
      Integer revision)
      throws Exception {
   ...
   * @throws Exception
   */
  public Swagger3VO genarateSwaggerJsonDefinations(Swagger3VO swaggerVO, List<RowData> rowDataList,
      Integer revision)
      throws Exception {
    log("genarateSwaggerJsonDefinations", swaggerVO.getInteractionid(), swaggerVO);
  ...
  private String populateSchemas(String swagger, Map<String, Schema> schemas) {
    ...
    try {
      ...
      String swaggerStr = mapper.writeValueAsString(objNode)
    ...
    for (String teamName : swaggerReview.getTeamNames()) {
      ...
      if (team != null) {
        ...
        emailTemplate.setSubject(
            "-- Subject : Review Alert - for the following Swagger :"
                + swaggerReview.getSwaggerName());
   ...
   */
  public void createOrUpdateReviewComment(SwaggerReviewComents swaggerReviewComents)
      throws ItorixException {
    log("createOrUpdateReviewComment", swaggerReviewComents.getInteractionid(),
        swaggerReviewComents);
    if (swaggerReviewComents.getExternalFlag() != null && swaggerReviewComents.getExternalFlag()
        .equals("true")) {
   ...
   * @param swaggerReviewComents
   */
  public void createOrUpdateReviewComment(Swagger3ReviewComents swaggerReviewComents)
      throws ItorixException {
    log("createOrUpdateReviewComment", swaggerReviewComents.getInteractionid(),
        swaggerReviewComents);
    swaggerReviewComents = baseRepository.save(swaggerReviewComents);
  ...
  public void updateReviewComment(Swagger3ReviewComents swaggerReviewComents) throws Exception {
    log("updateReviewComment", swaggerReviewComents.getInteractionid(), swaggerReviewComents);
    Swagger3ReviewComents sc = baseRepository.findById(swaggerReviewComents.getId(),
        Swagger3ReviewComents.class);
    if (sc != null) {
      sc.setComment(swaggerReviewComents.getComment());
      swaggerReviewComents = baseRepository.save(sc);
    } else {
  ...
  private List<SwaggerReviewComents> getReviewComments(SwaggerReviewComents sc) {
    ...
    if (swaggerVO != null) {
      List<SwaggerReviewComents> list = baseRepository.find("swaggerName", swaggerVO.getName(),
          "revision",
  ...
  private List<Swagger3ReviewComents> getReviewComments(Swagger3ReviewComents sc) {
    ...
    if (swaggerVO != null) {
      List<Swagger3ReviewComents> list = baseRepository.find("swaggerName", swaggerVO.getName(),
          "revision",
  ...
  private void log(String methodName, String interactionid, Object... body) {
    ...
    logger.debug("SwaggerService." + methodName + " | CorelationId=" + interactionid
  ...
  public SwaggerObjectResponse getSwaggerStats(String timeunit, String timerange)
    ...
    AggregationResults aggregationResults = baseRepository.addAggregation(aggregation,
        "Design.Swagger.List",
        SwaggerVO.class);
    ArrayList<Document> documentlist = (ArrayList) aggregationResults.getRawResults()
        .get("results");
   ...
   * @throws ParseException
   */
  public ObjectNode getSwagger3Stats(String timeunit, String timerange)
      throws ParseException, ItorixException {
    ...
    List<String> distinctList = baseRepository.findDistinctValuesByColumnName(Swagger3VO.class,
        STATUS_VALUE);
   ...
   * @throws ParseException
   */
  public ObjectNode getTeamStats(String timeunit, String timerange)
      throws ParseException, ItorixException {
    ...
    List<String> distinctList = baseRepository.findDistinctValuesByColumnName(SwaggerTeam.class,
        "name");
    if (distinctList != null && distinctList.size() > 0) {
   ...
   * @throws ItorixException
   */
  public void associateProduct(String swaggerName, Set<String> productSet, String oas)
      throws ItorixException {
   ...
   * @return
   */
  public SwaggerTeam findSwaggerTeam(String team_name, String interactionid)
      throws ItorixException {
  ...
  public void updateSwagger3BasePath(String name, Swagger3VO vo) {
    ...
    SwaggerParseResult swaggerParseResult = new OpenAPIParser().readContents(vo.getSwagger(), null,
        null);
    ...
    for (Server server : servers) {
      ...
      try {
        ...
        logger.error("Error while getting basePath for Swagger: {} URL {} ", vo.getName(),
            e.getMessage());
      }
    }
  ...
  public Object swaggerSearch(String interactionid, String name, int limit)
    ...
    List<String> allSwaggers = getList(
        mongoTemplate.getCollection("Design.Swagger.List").distinct("name",
  ...
  public Object swagger3Search(String interactionid, String name, int limit)
    ...
    query.limit(limit > 0 ? limit : 10);
    List<String> allSwaggers = getList(
        mongoTemplate.getCollection("Design.Swagger3.List").distinct("name",
            query.getQueryObject(), String.class));
    Collections.sort(allSwaggers);
    ObjectMapper mapper = new ObjectMapper();
    ObjectNode response = mapper.createObjectNode();
   ...
   * @return
   */
  public void createOrUpdateGitIntegrations(String interactionid, String jsessionid,
      String swaggerid, String oas,
    ...
    SwaggerIntegrations integrations = baseRepository.findOne("swaggerName", swaggerName, "oas",
        oas,
   ...
   * getGitIntegrations
   */
  public SwaggerIntegrations getGitIntegrations(String interactionid, String jsessionid,
      String swaggerid, String oas)
      throws ItorixException {
    String swaggerName = null;
    if (oas.equals("3.0")) {
   ...
   * @return
   */
  public void deleteGitIntegrations(String interactionid, String jsessionid, String swaggerid,
      String oas)
      throws ItorixException {
    String swaggerName = null;
    if (oas.equals("3.0")) {
  ...
  private String cloneSwagger2(SwaggerCloneDetails swaggerCloneDetails) throws ItorixException {
    ...
    if (null != swaggerCloneDetails.getRevision()) {
      vo = baseRepository.findOne("swaggerId", swaggerCloneDetails.getCurrentSwaggerID(),
          "revision",
  ...
  public void updateSwaggerDictionary(SwaggerDictionary swaggerDictionary) {
    SwaggerDictionary swaggerDictForUpdate = baseRepository.findOne("swaggerId",
        swaggerDictionary.getSwaggerId(),
        "revision", swaggerDictionary.getRevision(), "oasVersion",
        swaggerDictionary.getOasVersion(),
  ...
  public SwaggerDictionary getSwaggerDictionary(String swaggerId, Integer revision) {
    return baseRepository.findOne("swaggerId", swaggerId, "revision", revision,
        SwaggerDictionary.class);
  }

  @Override
  public DictionarySwagger getSwaggerAssociatedWithDictionary(String dictionaryId,
      String schemaName) {
