...
@Component
public class SupportedCodeGenLangUtil {
	...
	public List<String> getSupportedLanguages(String type,String oas){
		...
		if(oas.isEmpty() || oas.toLowerCase().contains("2")){
			supportedLanguages.addAll(mongoTemplate.find(
					new Query(
							Criteria.where("oas2Compatible").is(true)
									.and("type").is(type)
					),SupportedCodeGenLang.class));
		}else{
			supportedLanguages.addAll(mongoTemplate.find(
					new Query(
							Criteria.where("oas3Compatible").is(true)
									.and("type").is(type)
					),SupportedCodeGenLang.class));
		}
