...
public class GitIntegrationServiceImpl implements GitIntegrationService {
	...
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Get Git hub user Repos sucessfully", response = Object.class),
			@ApiResponse(code = 400, message = "Bad Request", response = ErrorObj.class),
			@ApiResponse(code = 500, message = "System Error", response = ErrorObj.class)})
	...
	public ResponseEntity<List<GitHubUserReposResponse>> gitHubUserRepos(
			...
			@RequestHeader(value = "JSESSIONID") String jsessionid, @RequestHeader(value = "token") String token)
			throws Exception {
		List<GitHubUserReposResponse> gitHubUserReposResponse = gitIntegrationBusiness.gitHubUserRepos(interactionid,
				jsessionid, token);
