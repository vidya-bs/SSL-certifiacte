...
public class TestSuiteStatsImpl {
    ...
    private void calculateAvgDuration(TestSuiteExecutionMetric executionMetric) {
        ...
        List<TestSuiteResponse> testSuiteResponses = mongoTemplate.find(query, TestSuiteResponse.class, TEST_EVENT_HISTORY);
    ...
    private List<TestSuiteExecutionMetric> getTestSuiteExecMetrics(String userId) {
        ...
        GroupOperation groupOperation = Aggregation.group("$testSuite.name").count().as("totalCount").push("$$ROOT").as("docs");

        ProjectionOperation projectionOperation = Aggregation.project("_id", "totalCount").
                and(filter("docs").as("doc").by(valueOf("doc.testSuite.status").equalToValue("PASS"))).as("successfulExecCount").
                and(filter("docs").as("doc").by(valueOf("doc.testSuite.status").equalToValue("FAIL"))).as("failureExecCount");

        ProjectionOperation projectCount = Aggregation.project("totalCount").and("$successfulExecCount").size().as("successfulExecCount").
                and("$failureExecCount").size().as("failureExecCount");

        AggregationResults<Document> results = null;

        if(userId != null ) {
            MatchOperation matchOperation = Aggregation.match(Criteria.where("userId").is(userId));
            results = mongoTemplate.aggregate(Aggregation.newAggregation(matchOperation, groupOperation, projectionOperation, projectCount), TEST_EVENT_HISTORY, Document.class);
        } else {
            results = mongoTemplate.aggregate(Aggregation.newAggregation(groupOperation, projectionOperation, projectCount), TEST_EVENT_HISTORY, Document.class);
        }
