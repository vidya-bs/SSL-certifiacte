...
public class LandingPageStatsImpl {
	...
	private Criteria getCriteriaForUser(String userId) {
		long unixEpoch = LocalDateTime.now().minusDays(5).toInstant(ZoneOffset.UTC).toEpochMilli();
		Criteria criteria = Criteria.where("id_user").is(userId).and("operation").is("POST").and("statusCode").is(200)
				.and("last_Changed_At").gte(unixEpoch);
		return criteria;
	}

	private Criteria getCriteriaWithoutUser() {
		long unixEpoch = LocalDateTime.now().minusDays(5).toInstant(ZoneOffset.UTC).toEpochMilli();
		Criteria criteria = Criteria.where("operation").is("POST").and("statusCode").is(200).and("last_Changed_At")
				.gte(unixEpoch);
		return criteria;
	}
	...
	public WorkspaceDashboard getWorkspaceDashboard(String userId) {
		...
		return mongoTemplate.findOne(Query.query(Criteria.where("userId").is(userId)).limit(1)
				.with(Sort.by(Sort.Direction.DESC, "createdTs")), WorkspaceDashboard.class);
	}
}
