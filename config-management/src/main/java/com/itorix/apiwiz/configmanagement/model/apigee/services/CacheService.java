...
public class CacheService {
	...
	public String getCacheURL(CacheConfig config) throws ItorixException{
		try{
			...
			String URL = apigeeUtil.getApigeeHost(config.getType()==null?"saas":config.getType(), org) + "v1/organizations/" + org + "/environments/" + env + "/caches?name=" + name;
			return URL;
		}
		catch( Exception ex){
	...
	public String getUpdateCacheURL(CacheConfig config) throws ItorixException{
		try{
			...
			String org = config.getOrg();
			String name = config.getName();
			String URL = apigeeUtil.getApigeeHost(config.getType()==null?"saas":config.getType(), org) + "v1/organizations/" + org + "/environments/" + env + "/caches/" + name;
			return URL;
		}
		catch( Exception ex){
			throw new ItorixException(ex.getMessage(),"Config-1000", ex );
