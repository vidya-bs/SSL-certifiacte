...
public class PerformanceAndCoveragePostmanCollectionRunner {
	...
	public List<Object> executePostManCollectionTraceAsObject(String postManFileName,String envFileName,CommonConfiguration cfg) throws Exception {
		try {
	...
	public List<Object> executePostManCollectionTraceAsObject(InputStream postManFileName,InputStream envFileName,CommonConfiguration cfg) throws Exception {
	...
	public PostmanRunResult runCollection(PostmanCollection collection, PostmanEnvironment env, String folderName,
		...
		if (c.folders != null && c.folders.size()>0) {
			...
			/*for (PostmanFolder pf : c.folders) {
				isSuccessful = runFolder(haltOnError, runner, var, c, null, runResult) && isSuccessful;
				if (haltOnError && !isSuccessful) {
					return runResult;
				}
			}*/
	...
	public List<Object> runCollectionTraceAsObject(PostmanCollection collection, PostmanEnvironment env, String folderName,
			boolean haltOnError,CommonConfiguration cfg) throws Exception {
	...
	private List<Object> runFolderTraceAsObject(boolean haltOnError, PostmanRequestRunner runner, PostmanVariables var,
			PostmanCollection c, PostmanFolder folder, PostmanRunResult runResult,CommonConfiguration cfg) throws ItorixException {
		logger.debug("==> POSTMAN Folder: " + folder.name);
		boolean isSuccessful = true;
		List<Object> traceList=new ArrayList<>();
		for (String reqId : folder.order) {
			...
			try {
				...
				if (sessionID != null) {
					...
					logger.debug("executeCodeCoverage"+ cfg.getInteractionid()+ " step4: getTransactionIds ::" + txIds);
				}
	...
	/*private boolean runFile(boolean haltOnError, PostmanRequestRunner runner, PostmanVariables var, PostmanCollection c, PostmanRunResult runResult) {
		boolean isSuccessful = true;
	...
	private List<Object> runFileTraceAsObject(boolean haltOnError, PostmanRequestRunner runner, PostmanVariables var, PostmanCollection c, PostmanRunResult runResult,CommonConfiguration cfg) throws ItorixException {
		List<Object> traceList=new ArrayList<>();
		if(c.order!=null && c.order.size()>0){
			for(String id:c.order){
				for (PostmanRequest req : c.requests) {
					if(id.equals(req.id)){
						...
						try {
							...
							if (sessionID != null) {
								...
								logger.debug("executeCodeCoverage"+ cfg.getInteractionid()+ " step4: getTransactionIds ::" + txId);
							}
							if(cfg.isCodeCoverage()){
							...
							if(cfg.isPolicyPerformance()){
								List<String> tracesObjects = commonServices.getTransactionDataAsString(cfg, sessionID, txId);
								traceList.addAll(tracesObjects);
							}
							String sessionStatus = commonServices.deleteSession(cfg, sessionID);
							logger.debug("sessionStatus :"+sessionStatus);
